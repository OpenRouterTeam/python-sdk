...
class OutputBashServerToolItemTypedDict(TypedDict):
    ...
    r"""The error message when the sandbox call failed before producing a result (for example, the per-user container limit was reached). Set together with `status: 'failed'`; absent on a successful call. A non-zero `exitCode` is a command failure, not a tool failure."""
